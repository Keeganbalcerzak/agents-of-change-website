...
(async () => {
  ...
  const y = await page.evaluate(() => {
    ...
    return el ? el.getBoundingClientRect().top + window.scrollY : 6200;
  });
  await page.evaluate((scrollY) => window.scrollTo(0, scrollY), y + 250);
  await page.waitForTimeout(1200);
  await page.screenshot({ path: path.join(outDir, 'verify-pricing-fix-3.png') });
  await browser.close();
})();
